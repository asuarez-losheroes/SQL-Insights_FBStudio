...
export default function GruposSoportePage() {
  ...
  const onSubmit = (data: Omit<FormData, 'id'>) => {
    if (editingGrupo) {
      updateRelationalData('gruposSoporte', { ...editingGrupo, ...data });
    } else {
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Administra los grupos de soporte.</CardDescription>
          </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <form onSubmit={handleSubmit(onSubmit)}>
            <DialogHeader>
              <DialogTitle>{editingGrupo ? "Editar Grupo" : "Añadir Grupo"}</DialogTitle>
            </DialogHeader>
